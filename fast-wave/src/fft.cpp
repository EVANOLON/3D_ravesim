...
template <typename S> FFT<S>::FFT(std::size_t N) : N(N), nx(0), ny(0), is_2d(false) {
...
//2d start
template <typename S>FFT<S>::FFT(std::size_t nx, std::size_t ny) 
    : nx(nx), ny(ny), N(nx * ny), is_2d(true) {
    const auto plan_type = std::is_same_v<S, double> ? CUFFT_Z2Z : CUFFT_C2C;
    const auto res = cufftPlan2d(&this->fft_plan, ny, nx, plan_type);
    check_cufft_result("cufftPlan2d", res);
}
//2d end
...
void FFT<S>::inverse(DevComplex<S> *d_U, DevComplex<S> *d_u) const {
    ...
    //2d before const Complex<S> norm_factor = 1. / this->N;
    //2d start
    const Complex<S> norm_factor = is_2d ? 
        Complex<S>(1.0 / (nx * ny)) : 
        Complex<S>(1.0 / N);
    //2d end
