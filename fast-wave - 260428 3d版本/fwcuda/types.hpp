...
struct SimParams {
    ...
    // 维度标志（内部计算，不暴露给外部）
    bool is_2d() const {
        return nx.has_value() && ny.has_value();
    }
    
    // 获取有效参数（向后兼容）
    int get_nx() const { return nx.value_or(N); }
    int get_ny() const { return ny.value_or(1); }
    double get_dy() const { return dy.value_or(dx); }
    double get_detector_size_x() const { return detector_size_x.value_or(detector_size); }
    double get_detector_size_y() const { return detector_size_y.value_or(detector_pixel_size_y); }
    
    // 获取总点数（考虑2D）
    int get_total_points() const {
        if (is_2d()) {
            return get_nx() * get_ny();
        }
        return N;
    }
    ...
    bool validate() const {
        ...
        if (dx <= 0) {
            std::cerr << "Error: dx must be positive" << std::endl;
            return false;
        }
        if (wl <= 0) {
            std::cerr << "Error: wavelength must be positive" << std::endl;
            return false;
        }
        
        // 检查2D参数一致性（如果提供）
        if (nx.has_value() && nx.value() <= 0) {
            std::cerr << "Error: nx must be positive if provided" << std::endl;
            return false;
        }
        if (ny.has_value() && ny.value() <= 0) {
            std::cerr << "Error: ny must be positive if provided" << std::endl;
            return false;
        }
        if (dy.has_value() && dy.value() <= 0) {
            std::cerr << "Error: dy must be positive if provided" << std::endl;
            return false;
        }
    ...
    void print_info(const std::string& prefix = "") const {
        ...
        if (is_2d()) {
            std::cout << prefix << "  Grid: nx=" << *nx << ", ny=" << *ny 
                      << ", total=" << (*nx * *ny) << std::endl;
            std::cout << prefix << "  Pixel size: dx=" << dx << " m, dy=" << *dy << " m" << std::endl;
            std::cout << prefix << "  Detector size: " << get_detector_size_x() * 1e6 
                      << " μm x " << get_detector_size_y() * 1e6 << " μm" << std::endl;
        } else {
            std::cout << prefix << "  Grid: N=" << N << " (1D)" << std::endl;
            std::cout << prefix << "  Pixel size: dx=" << dx << " m" << std::endl;
            std::cout << prefix << "  Detector size: " << detector_size * 1e6 << " μm" << std::endl;
        }
