...
def process_txt_file(file_path, sigma=0.1):
    ...
    noise=np.random.normal(0, sigma*np.average(data_1d), data_1d.shape)
    # 添加噪声
    noisy_data = data_1d + noise
...
if __name__ == "__main__":
    ...
    for ii in times:
        print(ii)
        input_file = 'D1720_260209_output_shocksample_shot_timestamp'+str(ii)+'.txt'  # 输入文件路径
    ...
    plt.tight_layout()
    plt.show()
    save_results(total, output_file='SC_result.npy', output_txt='SC_result.txt')