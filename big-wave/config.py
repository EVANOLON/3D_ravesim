...
def parse_optical_element(
    ...
    if dct["type"] == "env_grating":
        ...
        grid = np.load(config_dir / dct["grid_path"],allow_pickle=True)
        ...
        sample_materials = [parse_material(m) for m in dct["materials"]]
        material_grid = np.load(config_dir / dct["material_grid_path"])
        density_grid = np.load(config_dir / dct["density_grid_path"])
