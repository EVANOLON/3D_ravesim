...
class SimParams:
    ...
    @property
    def nx(self) -> int:
        return self.N // self.ny if self.is_2d else self.N
...
def apply_frequency_cutoff_2d(
    ...
    ky2 = np.zeros(ny, dtype=np.float64)
    for ix in range(nx):
        kx = fftfreq_1d(nx, ix) / dx
        kx2[ix] = kx * kx
    for iy in range(ny):
        ky = fftfreq_1d(ny, iy) / dy
        ky2[iy] = ky * ky
...
def propagate_2d(
    ...
    # Pre-compute kx² and ky² lookup tables for efficiency
    kx2 = np.zeros(nx, dtype=np.float64)
    ky2 = np.zeros(ny, dtype=np.float64)
    for ix in range(nx):
        kx = fftfreq_1d(nx, ix) / dx
