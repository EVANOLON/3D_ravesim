...
def run_single_simulation(
    ...
    if history is not None:
        ...
        if sim_params.is_2d:
            np.save(sub_dir / "history_x.npy",
                    np.arange(sim_params.nx) * sim_params.dx)
            np.save(sub_dir / "history_y.npy",
                    np.arange(sim_params.ny) * sim_params.get_dy())
        else:
            np.save(
...
def setup_simulation(dct: config.DictType, config_dir: Path, save_dir: Path) -> Path:
    ...
    if "nx" in dct["sim_params"] and "ny" in dct["sim_params"]:
        # 2D 仿真：N 是总像素数，dx 是 x 方向的间距，使用 nx 进行解析传播检查
        Nx = dct["sim_params"]["nx"]
    else:
        Nx = sim_params.N
# -----------------------------------------
    ...
    if multisource["type"] == "points":
        ...
        if sim_params.is_2d and not use_fresnel:
            ...
            grid_density_check_2d(
                ...
                max(abs(x_range[0]), abs(x_range[1])), sim_params.ny, sim_params.get_dy(), wl_e_max,
            )
