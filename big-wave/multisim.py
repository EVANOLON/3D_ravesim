...
# -----------------------------------------
    ...
    if multisource["type"] == "points":
        ...
        max_y = max(abs(y_range[0]), abs(y_range[1]))
    ...
    if not use_fresnel:
        ...
        #3d adjustment end
        if sim_params.is_2d:
            max_y_det = max_y_list[-1]
            required_Ny = int(np.ceil(max_y_det / sim_params.get_dy()) * 2)
            assert sim_params.ny >= required_Ny, (
                f"Reflections at the boundary might occur: ny={sim_params.ny}, "
                f"required_Ny={required_Ny}, max_y={max_y_det}, dy={sim_params.get_dy()}"
            )
