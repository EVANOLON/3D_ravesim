...
class Vector(Protocol):
    ...
    def fft2(self: Self, destination: Self, nx: int, ny: int):
        """Perform a 2D forward fourier transform (row-major: ny rows, nx cols)"""
        raise NotImplementedError("fft2 not implemented for this vector type")

    def ifft2(self: Self, destination: Self, nx: int, ny: int):
        """Perform a 2D inverse fourier transform (row-major: ny rows, nx cols)"""
        raise NotImplementedError("ifft2 not implemented for this vector type")
...
class NumpyVector(Vector):
    ...
    def fft2(self: Self, destination: Self, nx: int, ny: int):
        mat = self.vec.reshape(ny, nx)
        destination.vec = spfft.fft2(mat).reshape(-1)

    def ifft2(self: Self, destination: Self, nx: int, ny: int):
        mat = self.vec.reshape(ny, nx)
        destination.vec = spfft.ifft2(mat).reshape(-1)
